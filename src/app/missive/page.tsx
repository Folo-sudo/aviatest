...
import { getMissiveCount, submitMissive } from '@/lib/feedback/api';
    ...
    <main className="min-h-screen" style={{ backgroundColor: styles.background }}>
      ...
      >
        <div className="container mx-auto px-4 py-4 flex items-center gap-3">
          ...
          <Mail className="h-5 w-5" style={{ color: styles.text }} />
          <h1 className="text-lg font-bold" style={{ color: styles.text }}>
            Missive au grand codeur
          </h1>
        </div>
      </header>
      ...
      <div className="container mx-auto px-4 py-8 max-w-xl">
        ...
        >
          <p className="text-sm" style={{ color: styles.textMuted }}>
            ...
            {remaining > 1 ? 's' : ''} restante{remaining > 1 ? 's' : ''} (max {LIMIT}).
          </p>
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            />
            {message && (
              <p className={`text-sm ${message.includes('envoyee') ? 'text-emerald-600' : 'text-red-500'}`}>
                {message}
              </p>
            )}
            <Button
